...
def add_or_update_parking_data(request, payload: CreateParkingDataSchema):
    ...
    old_records = ParkingData.objects.filter(sensor_id=payload.sensor_id).order_by(
        "-timestamp"
    )[1:]
    ...
    print(
        f"status of {payload.sensor_id} changed to {payload.parking_status} at {payload.sensor_location}"
    )
    return parking_data
